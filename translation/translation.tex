...
\begin{document}
    ...
    \begin{lemma} \label{lemma:4.4}
        ...
        have the same color. \pdfcomment[icon=Note,
        color=yellow]{
    ...
    \begin{proof}
        ...
        connected. \pdfcomment[icon=Note, color=yellow]
        {if $G\left(F\right)$ was
        ...
        \pdfcomment[icon=Note, color=yellow]{That would imply
        that $i = 1$ or $i = t$ 
        for all $i \in
        \left\{1, \ldots, t\right\}$
        and so $t \leq 2$}
        ...
        the colorings $f_1$ and 
        $f_2$ can be fused into 
        one coloring $f$.}
        ...
        we obtain a 
        new coloring $f'$ of 
        $F \setminus \left\{h_2\right\}$. \pdfcomment[icon=Note,
        color=yellow]{
        We essentially flip 2 and $\delta$,
        if there was no conflict
        before, there is no conflict
        now} Since $h_3 \in F^{+}\left(b_2'\right)$,
        \pdfcomment[icon=Note, color=yellow]
        {since $h_2'$ and $h_3$ do not
        overlap} we get
        ...
        required by the present lemma.
        \pdfcomment[icon=Note, color=yellow]
        {this is because $F^{0}\left(a_2\right)
        = \left\{h_1\right\}$ and
        $F^{0}\left(b_2\right) = \left\{h_3\right\}$}
    \end{proof}
    ...
    \begin{proof}
        ...
        on $k$. \pdfcomment[icon=Note, color=yellow]
        {$k$ is the 
        number of ``layers''}
        Let $k = 1$.
        ...
        \begin{enumerate}
            ...
            \item \label{cond:3} All intervals in 
                ...
                \pdfcomment[icon=Note, color=yellow]{For 
                $k = 1$, this is
                Lemma \ref{lemma:4.5}}
            \item For each $p =
                \left(c, d\right) \in 
        ...
        of $F \setminus F'$. \pdfcomment[icon=Note, color=yellow]{by 
        condition \ref{cond:3}}
        Let
        \begin{align*}
        ...
        family consists of
        nested intervals.
        \pdfcomment[icon=Note, color=yellow]{otherwise
        $\omega\left(F\right) \geq 3$}
        Without loss of generality,
        suppose that
        ...
        \pdfcomment[icon=Note, color=yellow]{Could also be
        only $\gamma_2$}
        ...
        \pdfcomment[icon=Note, color=yellow]{The following
        levels can only 
        overlap with 
        $\left\{h_{t+1},
        \ldots, h_{s}\right\}$}
        ...
        \pdfcomment[icon=Note, color=yellow]{Actually 
        $F^{0}\left(d_{s} + 1\right)$ 
        is empty, but we want
        to apply the statement
        of Lemma \ref{lemma:4.4}
        rigorously. This
        is why we introduce
        the segment $\left(d, d_{s} + 1\right)$ 
        in the first place.}
        we can assume that 
        $h_{t+1}, \ldots,
        h_{s}$ is colored 
